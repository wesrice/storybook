...
export const controlOrMetaKey = () => (isMacLike() ? '⌘' : 'ctrl');
export const optionOrAlt = () => (isMacLike() ? '⌥' : 'alt');
...
export const getPrevious = ({ path, dataset }) => {
  ...
  if (index === 0) {
    if (parent && parent.isRoot) {
      console.log('ROOTROOTROOTROOTROOTROOTROOTROOTROOTROOTROOTROOTROOTROOTROOTROOT');
...
export const getNext = ({ path, dataset }) => {
  ...
  if (children && children.length && (current.isExpanded || current.isRoot)) {
  ...
  console.log('getNext : next', next, 'next.result', next.result);
  return next.result;
};
